...
def parse_llm_thought(full_text: str) -> tuple[str, str]:
    """
    Extracts the <think> reasoning and the final answer from LLM output.

    Args:
        full_text: The raw text generated by the model.

    Returns:
        A tuple of (thinking_process, final_answer).
    """
...
def build_context_from_chunks(chunks: list[Chunk]) -> str:
    """
    Formats a list of retrieved chunks into a context string for the LLM.

    Args:
        chunks: List of chunk metadata and text.

    Returns:
        A single formatted string with source identifiers.
    """
    context = []
